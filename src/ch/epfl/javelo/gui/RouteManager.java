...
public class RouteManager {
    ...
    public RouteManager(RouteBean routeBean, ObjectProperty<MapViewParameters> mapViewParametersProperty){
        ...
        if(routeBean.getRouteProperty() != null) drawRouteAndCircle();
        ...
        routeBean.getRouteProperty().addListener((p, oldS, newS) -> drawRouteAndCircle());

        routeBean.highlightedPositionProperty().addListener((p, oldS, newS) -> drawRouteAndCircle());
    ...
    private void addWaypointInTheWhiteCircle(double x, double y) {
        if(routeBean.getRouteProperty().getValue() == null) return;
        ...
        PointWebMercator pwm = PointWebMercator.of(mapViewParametersProperty.get().zoomLevel(),
                p2dLocalToParent.getX(),
                p2dLocalToParent.getY()) ;

        Waypoint wp = new Waypoint(pwm.toPointCh(),
                routeBean.getRouteProperty().get().nodeClosestTo(routeBean.highlightedPosition()));

        int indexOfCircleOnSegment = routeBean.indexOfNonEmptySegmentAt(routeBean.highlightedPosition()) + 1;
        routeBean.setPoint(indexOfCircleOnSegment, wp);
